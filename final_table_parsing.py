...
def get_value_next_to_header(df, keyword):
    ...
    for i in range(len(df)):
        ...
        for col_idx, cell_val in enumerate(row):
            if cell_val:
                # 📍 핵심: 셀 데이터에서도 모든 공백과 줄바꿈을 제거 후 비교
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for p_idx, page in enumerate(pdf.pages):
            ...
            for table in tables:
                ...
                #         # 값이 None이거나 빈칸이 아닐 때만 출력 (보기 편하게)
                #         if value is not None and str(value).strip() != "":
                #             clean_text = str(value).replace('\n', ' ')
                #             print(f"📍 [행 {r_idx}, 열 {c_idx}] ➔ {clean_text}")
                #         print("-" * 30) # 행 구분을 위한 선
                ...
                if len(df.columns) >= 10:
                    ...
                    if start_idx != -1:
                        ...
                        for _, row in occ_df.iterrows():
                            ...
                            # --- [좌표 자동 보정 로직] ---
                            if len(row_list) > 3 and row_list[3] != "": # 정상 구조 (좌표 분석 결과 기반)
                            ...
                            else: # 페이지가 바뀌어 컬럼이 밀리거나 축소된 경우 (Index 보정)
                                # row_list의 길이가 짧아진 경우를 대비하여 안전하게 접근
                                def get_val(idx): return row_list[idx] if idx < len(row_list) else ""
                ...
                for i in range(len(df)):
                    ...
                    if "등기된 부동산에 관한 권리 또는 가처분으로 매각으로" in full_row_text:
                        if i + 1 < len(df):
                            ...
                            if content:
                                result["surviving_rights"].append(content)
